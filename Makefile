...
format: check-venv ## Format the code
	...
	$(environment) ruff format src tests
	$(environment) ruff check src tests --fix


lint: check-venv ## Lint the code
	@printf "$(CYAN)Auto-formatting with ruff$(COFF)\n"
	$(environment) ruff check src tests
	$(environment) mypy src tests


pytest: check-venv ## Runs the test suite.
	@printf "$(CYAN)Running test suite...$(COFF)\n"
	$(environment) pytest $${TEST_ARGS:-"tests/"}